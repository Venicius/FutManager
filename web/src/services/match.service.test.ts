...
describe("match.service", () => {
  ...
  it("deve criar uma nova partida e retornar o id", async () => {
    ...
    const id = await createMatch("Pelada de Sábado", "2026-03-07T15:00:00");
  ...
  it("deve listar partidas ordenadas", async () => {
    const mockDocs = [
      { id: "1", data: () => ({ titulo: "Partida 1", data: "2026-03-01T10:00:00", presentPlayers: ["player1"] }) },
    ];
  ...
  it("deve marcar presença (arrayUnion)", async () => {
    ...
    await toggleAttendance("match-1", "player-1", true);

    expect(doc).toHaveBeenCalledWith(expect.anything(), "partidas", "match-1");
  ...
  it("deve desmarcar presença (arrayRemove)", async () => {
    (doc as any).mockReturnValue("docRef-mock");
    await toggleAttendance("match-1", "player-1", false);
  ...
  it("deve atualizar os dados de uma partida", async () => {
    ...
    await updateMatch("match-1", { titulo: "Novo Titulo" });
