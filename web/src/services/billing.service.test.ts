...
describe("billing.service", () => {
  ...
  describe("generateMonthlyBillings", () => {
    it("deve abater valor total se jogador tiver crédito suficiente", async () => {
      ...
      const gerados = await generateMonthlyBillings(50, "2026-03-05");

      expect(gerados).toBe(1);
      // Deve ter criado cobrança como PAGO na coleção cobrancas
      expect(addDoc).toHaveBeenCalledWith("cobrancas", expect.objectContaining({
        status: "PAGO",
        valor: 50
      }));
      // Deve ter atualizado o jogador para crédito de 10
      expect(updateDoc).toHaveBeenCalledWith("jogadores/p1", { creditoAcumulado: 10 });
    });
    ...
    it("deve abater parcial se jogador tiver crédito menor que o valor", async () => {
      ...
      expect(updateDoc).toHaveBeenCalledWith("jogadores/p2", { creditoAcumulado: 0 });
    });
  });

  describe("payBilling", () => {
    it("deve gerar crédito se pagar a mais", async () => {
      const cobranca = { id: "c1", jogadorId: "p1", nomeJogador: "User 1", valor: 50, vinculo: "Mensalista", referencia: "03/2026" };
      (getPlayers as any).mockResolvedValue([{ id: "p1", creditoAcumulado: 0 }]);
      
      await payBilling("c1", cobranca as any, 70);

      // Cobrança fica PAGO
      expect(updateDoc).toHaveBeenCalledWith("cobrancas/c1", { status: "PAGO" });
      // Adiciona 20 de crédito
      expect(updateDoc).toHaveBeenCalledWith("jogadores/p1", { creditoAcumulado: 20 });
      // Registra transação de 70 reais
      expect(addTransaction).toHaveBeenCalledWith(expect.objectContaining({ amount: 70 }));
    });

    it("deve abater valor pendente se pagar a menos", async () => {
      const cobranca = { id: "c2", jogadorId: "p2", nomeJogador: "User 2", valor: 50, vinculo: "Mensalista", referencia: "03/2026" };
      
      await payBilling("c2", cobranca as any, 30);

      // Apenas o valor da cobrança muda para 20
      expect(updateDoc).toHaveBeenCalledWith("cobrancas/c2", { valor: 20 });
      // Registra transação de 30 reais
      expect(addTransaction).toHaveBeenCalledWith(expect.objectContaining({ amount: 30 }));
    });
  });
  ...
  describe("generateMatchBillings", () => {
    it("deve criar cobrança apenas para os Diaristas presentes", async () => {
      ...
      const gerados = await generateMatchBillings(matchId, matchTitle, presentPlayerIds, allPlayers, amount, dueDate);
